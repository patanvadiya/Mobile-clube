...
<body>
		...
		<div class="container mt-3">
			<h1 class="text-center">Realme Mobile</h1>
			...
			<div class="row mt-5" id="card">
				@foreach($data as $realme)
				<div class="col-sm-3" >
					<div class="card">
						<div class="card-header">{{$realme->title}}</div>
						<div class="card-body">
							<textarea style="display:none;">{{$realme->title}} {{$realme->company}}</textarea>
							<img src="{{('storage/'.$realme->image)}}" style="height:200px;">
						</div>
					</div>
					<a href="{{url('addCart/'.$realme->id.'/'.$realme->type)}}" class="btn btn-warning mt-2">Add Cart</a>
				</div> 
				@endforeach				
			</div>
		</div>
