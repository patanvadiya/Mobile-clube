...
<body>
		<div class="container mt-3">
			<h1 class="text-center">Oppo Mobile</h1>
			...
			<div class="row mt-5" id="card">
				@foreach($data as $oppo)
				<div class="col-sm-3">
					<div class="card">
						<div class="card-header">{{$oppo->title}} {{$oppo->company}}</div>
						<div class="card-body">
							<textarea style="display:none;">{{$oppo->title}} {{$oppo->company}}</textarea>
							<img src="{{('storage/'.$oppo->image)}}" style="height:200px;">
						</div>
					</div>
					<a href="{{url('addCart/'.$oppo->id.'/'.$oppo->type)}}" class="btn btn-info mt-2">Add Cart</a>
				</div> 
				@endforeach				
			</div>
		</div>
