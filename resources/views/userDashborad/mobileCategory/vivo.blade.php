...
<body>
		<div class="container mt-3">
			<h1 class="text-center">Vivo Mobile</h1>
			...
			<div class="row mt-5" id="card">
				@foreach($data as $vivo)
				<div class="col-sm-3">
					<div class="card">
						<div class="card-header">{{$vivo->title}} {{$vivo->company}}</div>
						<div class="card-body">
							<textarea style="display:none;">{{$vivo->title}} {{$vivo->company}}</textarea>
							<img src="{{('storage/'.$vivo->image)}}" style="height:200px;">
						</div>
					</div>
					<a href="{{url('addCart/'.$vivo->id.'/'.$vivo->type)}}" class="btn btn-warning mt-2">Add Cart</a>
				</div> 
				@endforeach				
			</div>
		</div>
